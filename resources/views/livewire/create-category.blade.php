 ...
 <div>
 <div wire:ignore.self class="modal fade lug" id="createCategoryModal" role="dialog" >
                 ...
                 <div class="wrapper wrapper--w780 ">
                    <div class="card card-3 border border-danger row w-100 p-0 m-0">
                         <div class="card-body border p-0 border-success col-12 col-lg-8 col-xl-8">
                            <h5 class="z-title text-white text-center p-1 m-0 "> Création d'une nouvelle catégorie
                                <br>
                            ...
                            <hr class="m-0 p-0 bg-info">
                                <form autocomplete="off" class="mt-3 pb-3" wire:submit.prevent="create">
                                    <div class="input-group zw-95 mt-0 mb-2">
                                        <label class="z-text-cyan @error('name') text-danger @enderror m-0 p-0 w-100 cursor-pointer" for="category_new_name">Le nom de la catégorie</label>
                                        <hr class="m-0 p-0 bg-info w-100 mb-1">
                                        <input class="input--style-3 px-2 @error('name') text-danger border border-danger @enderror" wire:model.defer="name" type="text" placeholder="Le nom de la catégorie" name="category_name" id="category_new_name">
                                        @error('name')
                                            <small class="py-1 text-warning">{{$message}}</small>
                                        @enderror
                                    </div>
                                    <div class="input-group mt-0 mb-2 zw-95">
                                        <label class="z-text-cyan @error('description') text-danger @enderror m-0 p-0 w-100 cursor-pointer" for="category_new_description">Décrivez brièvement la catégorie</label>
                                        <hr class="m-0 p-0 bg-info w-100 mb-1">
                                        <input class="input--style-3 px-2 @error('description') text-danger border border-danger @enderror" placeholder="La description de la catégorie" wire:model.defer="description" type="text" name="category_description" id="category_new_description">
                                        @error('description')
                                            <small class="py-1 text-warning">{{$message}}</small>
                                        @enderror
                                    </div>
